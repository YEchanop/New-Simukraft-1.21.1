...
城市升级的等级和资源定义来自数据包，完整 JSON 格式见[城市升级系统](城市升级系统.md)。调用层只消费快照，不应在 Java 代码中写死等级、资金、人口、物品或解锁容量。

| 类 | 方法/字段 | 用途 |
|---|---|---|
| `CityLevelDefinitionLoader` | `definitions()` / `definition(level)` | 读取当前已发布的不可变等级定义快照 |
| `CityLevelDefinitionLoader` | `nextLevel(currentLevel)` | 查询当前等级的连续下一级；不存在时返回 `null` |
| `CityLevelDefinitionLoader` | `futureLevels(currentLevel, limit)` | 获取客户端展示用的后续等级列表，结果有数量上限 |
| `CityUpgradeService` | `nextDefinition(city)` | 按城市当前等级查询可升级目标 |
| `CityUpgradeService` | `upgrade(level, player, city, expectedCurrentLevel, targetLevel)` | 服务端原子校验并开始升级任务，返回 `UpgradeResult` |
| `CityUpgradeService` | `tick(level)` | 服务端主线程完成已到期任务并应用等级解锁；已在 `SimuKraft.onServerTick` 接入 |
| `CityData` | `upgradeState()` | 读取进行中的 `CityUpgradeState` 不可变快照 |
| `CityUpgradeState` | `targetLevel()` / `startedAt()` / `durationTicks()` | 读取目标等级、开始时刻和总耗时；`active()` 判断是否进行中 |
| `CityNetworkViewFactory` | `buildOpenResponse(level, pos, viewerId)` | 组装城市核心界面快照，包含 `upgradeTargets` 与 `upgradeProgress` |
| `CityCoreOpenResponsePacket` | `upgradeTargets()` / `upgradeProgress()` | 客户端可展示的等级条件、解锁容量和升级计时快照 |
| `CityUpgradeRequestPacket` | `simukraft:city_upgrade_request` | 客户端提交城市核心位置、期望当前等级和目标等级 |
| `CityUpgradePanelFactory` | `create(packet)` | 客户端使用 LDLib2 ORE 主题构建升级页面；由城市核心界面调用 |

推荐调用流程：

```text
CityCoreOpenResponsePacket
  -> CityUpgradePanelFactory.create
  -> new CityUpgradeRequestPacket(pos, cityLevel, targetLevel)
...
- 客户端不能直接调用 `CityUpgradeService`、修改 `CityData`、扣除背包物品或访问 SQLite；升级按钮只发送 `CityUpgradeRequestPacket`。
- 服务端会重新校验城市核心访问权、官方权限、期望等级、连续下一级、SQLite 可用性、资金、实时人口和背包物品（包括物品标签），不能信任客户端快照。
- 资金和材料在升级任务开始时扣除；`unlocks.chunks` 与 `unlocks.enclaves` 只在任务完成后生效，飞地容量上限由等级定义校验为 15。
- `CityCoreOpenResponsePacket.UpgradeProgress` 仅用于客户端显示进度。服务端以 `startedAt + durationTicks` 判断完成，客户端到期后必须重新请求快照。
- 数据包热重载后应重新读取 `CityLevelDefinitionLoader` 的快照；不要缓存可变的等级定义对象。
...
| `BuildingCatalog.BuildingDefinition` | `unlockLevel()` | 读取 `.sk` 的建筑解锁等级；缺失或空白时为 0，表示无等级限制 |
| `BuildingStructureService` | `loadStructure/resolvePlacedBlocks` | 加载结构并解析旋转后的方块 |
| `BuildBoxStartConstructionPacket` | `simukraft:build_box_start_construction` | 开始施工请求；服务端会重新校验建筑解锁等级 |
...
- 建筑 `.sk` 中的 `unlockLevel`（兼容 `unlock_level`）表示最低城市等级；字段缺失或空白表示无等级限制。城市等级达到或高于该值才可建造，升级进行中仍按当前已完成等级判断。
- 建筑列表的锁定标识只是客户端展示；`BuildBoxStartConstructionPacket` 必须重新读取 `BuildingCatalog.BuildingDefinition.unlockLevel()` 并在服务端拒绝等级不足的请求。
