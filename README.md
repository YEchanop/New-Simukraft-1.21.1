...
- 在市民信息界面的「身份证」卡片底部和市民管理界面中均新增皮肤设置区，可查看当前皮肤、进入皮肤选择列表或刷新皮肤
...
- **所有玩家均可使用皮肤切换功能，无需任何权限**

#### 皮肤资源下载中心
- 在市民信息界面的皮肤选择界面和市民管理界面的皮肤弹窗中均新增「下载中心」，从皮肤目录 API 拉取全部皮肤，以缩略图列表展示供玩家自由下载
...
### 市民 AI 对话
- 在市民管理界面为每位市民新增「聊天」按钮，可与单个市民进行 AI 对话（所有玩家均可使用，无需任何权限）
- 打开对话框后自动向服务器请求市民上下文快照（姓名、职业、工作状态、性格摘要、爱好、家庭角色、最近事件），作为对话背景
- 支持多轮对话记忆、对方正在输入提示、停止生成、重新生成、清空上下文与切换模型（切换模型不丢失对话历史）
- 服务器提供全局开关 `enableCitizenAiChat`（默认开启），关闭后聊天入口与上下文请求均不可用
- 对话过程中的错误（认证失败 / 端点未找到 / 限流 / 网络错误 / 功能禁用等）均以友好提示显示，不崩溃

#### AI 域名/模型管理
- 客户端自备 OpenAI 兼容 API 域名（如 SenseNova、OpenAI 等），在市民管理顶栏「AI 设置」面板中配置
- 每个域名可设置别名、基础地址、API Key（默认密文显示、可切换可见）、启用状态，并支持连通性测试
- 可为每个域名维护多个模型：手动添加 modelId，或从 `/v1/models` 批量获取；每行可启用/停用、设为默认
- 域名与模型配置持久化在 `config/simukraft-client.toml` 的 `citizenAi` 小节；API Key 在日志与界面中均脱敏显示
